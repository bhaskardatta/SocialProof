...
def get_relevant_context(query: str) -> str:
    ...
    if any(
        word in query_lower
        for word in ["phishing", "email", "suspicious email", "fake email"]
    ):
    ...
    if any(
        word in query_lower
        for word in ["smishing", "sms", "text message", "text", "message"]
    ):
    ...
    if any(
        word in query_lower
        for word in ["social engineering", "manipulation", "trick", "scam", "fraud"]
    ):
...
PayPal Security Team""",
    ...
    system_message = SystemMessage(
        content="""You are an expert cybersecurity trainer creating realistic phishing email scenarios for educational purposes. 
...
Always include: From, Subject, and email body with a suspicious link or request."""
    ...
    human_message = HumanMessage(
        content=f"""Create a realistic {difficulty} difficulty phishing email scenario.
...
def digital_guardian_query(question: str) -> Dict[str, Any]:
    ...
    system_message = SystemMessage(
        content=f"""You are the Digital Guardian, an expert cybersecurity assistant specializing in:
